...
# Minor changes for passing internet site automatically for postfix installation

# This code is written by Luke Smith
...
debconf-set-selections <<< "postfix postfix/mailname string "$postintname
debconf-set-selections <<< "postfix postfix/main_mailer_type string 'Internet Site'"
apt-get install --assume-yes postfix

apt -y install dovecot-imapd dovecot-sieve opendkim spamassassin spamc
# Check if OpenDKIM is installed and install it if not.
which opendkim-genkey >/dev/null 2>&1 || apt -y install opendkim-tools
